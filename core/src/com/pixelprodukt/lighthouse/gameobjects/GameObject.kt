...
open class GameObject : Renderable, Updatable, Comparable<GameObject> {
    ...
    override fun render(batch: SpriteBatch) {
        val originX: Float = width.toFloat().div(2)
        val originY: Float = height.toFloat().div(2)
        batch.draw(region, x.toFloat(), y.toFloat(), originX, originY, width.toFloat(), height.toFloat(), 1.0f, 1.0f, 0.0f)
    }
